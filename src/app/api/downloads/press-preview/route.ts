...
export async function GET() {
  try {
    const zip = new JSZip()
    await addFolderToZip(zip, PRESS_PREVIEW_DIR, "previews")
    zip.file("iiode-01-abat-jour.txt", await readFile(PRESS_INFO_FILE))
    ...
    return NextResponse.json({ ok: false, error: "Press preview generation failed" }, { status: 500 })
  }
}

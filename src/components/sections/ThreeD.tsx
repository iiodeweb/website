  ...
  return (
    <section id='view-3d' className='bg-background'>
      <div className='iiode-section-wrap'>
        <div className='iiode-section-panel grid grid-cols-1 md:grid-cols-2 ' data-scroll-panel='true'>
          <div className='iiode-split-half order-2 relative md:order-1 min-h-[100vh] md:min-h-auto'>
            <div className='absolute inset-0 py-6'>
              <AnimatedExplodedSvg src={copy.animationSrc} posterSrc={copy.animationPoster} explodedPosterSrc={copy.explodedPosterSrc} durationSeconds={copy.durationSeconds} triggerProgress={copy.triggerProgress} onPlaybackComplete={() => setOverlayEnabled(true)} className='h-full w-full' />
            </div>

            <CompositionOverlay key={overlayEnabled ? 'enabled' : 'disabled'} enabled={overlayEnabled} partMarkers={partMarkers} items={copy.compositionItems} note={copy.compositionNote} />
          </div>
...
function CompositionOverlay({ enabled, partMarkers, items, note }: CompositionOverlayProps) {
  ...
  return (
    <div
      className={`absolute inset-0 z-10 transition-opacity duration-500 ${enabled ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`}
      onClick={(event) => {
    ...
    >
      ...
      {activePart !== null ? (
        <p className='iiode-type-2 pointer-events-none absolute left-1/2 z-20 w-[min(86%,42rem)] -translate-x-1/2 -translate-y-1/2 bg-background/80 p-4 text-center text-foreground' style={{ top: `${partMarkers[activePart]}%` }}>
          {items[activePart]}
        </p>
      ) : null}

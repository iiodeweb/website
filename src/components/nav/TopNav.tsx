      ...
      <div className='iiode-container relative flex h-16 items-center gap-6'>
        ...
        <button type='button' onClick={() => setIsMenuOpen((open) => !open)} className='ml-auto flex h-8 w-8 items-center justify-center transition-colors md:hidden' aria-expanded={isMenuOpen} aria-label='Toggle menu'>
          <span className='relative block h-3 w-5'>
            <span className={`absolute left-0 top-0 h-[2px] w-5 bg-current transition ${isMenuOpen ? 'translate-y-[5px] rotate-45' : ''}`} />
            <span className={`absolute left-0 top-[5px] h-[2px] w-5 bg-current transition ${isMenuOpen ? 'opacity-0' : ''}`} />
            <span className={`absolute left-0 top-[10px] h-[2px] w-5 bg-current transition ${isMenuOpen ? 'translate-y-[-5px] -rotate-45' : ''}`} />
          </span>
        </button>
      </div>
      {isMenuOpen ? (
